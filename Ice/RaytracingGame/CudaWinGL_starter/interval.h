...
public:
    ...
    __device__ interval() : min(0), max(1) {} // Default interval is empty
...
__device__ interval operator+(const interval& ival, double displacement) {
    return interval(ival.min + displacement, ival.max + displacement);
}

__device__ interval operator+(double displacement, const interval& ival) {
