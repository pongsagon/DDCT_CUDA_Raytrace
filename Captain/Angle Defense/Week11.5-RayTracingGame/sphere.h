...
public:
    ...
    spheres()
    {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            float theta = rand01() * 2 * M_PI;
            float phi = acos(2 * rand01() - 1);
            float r = boundaryRadius * cbrt(rand01());
    ...
    void moveSphere(int idx, vec3 displacement) {
        ...
        if (!isWithinBounds(new_position, boundaryCenter, boundaryRadius - radius[idx])) {
            ...
            if (idx != 0 && isApproachingSphere(center[idx], new_position, center[0], radius[0] + radius[idx])) { 
    ...
    void updateSpheres(float deltaTime, const vec3& playerPosition, const vec3& bossPosition) {
        static vec3 lastPlayerPosition = playerPosition;  
        float orbit_radius = 5.0; 
        float base_orbit_speed = 2.0f; 
        float friction = 0.98;
        ...
        for (int i = 2; i < list_size; i++) {
            if (isProjectile[i]) {
                ...
                if (spawning[i]) {
                    ...
                    float targetRadius = radius[i]; // Arbitrary target radius
                    radius[i] += (targetRadius / 1.0f) * deltaTime;

                    if (radius[i] >= targetRadius) {
                        radius[i] = targetRadius; 
                        spawning[i] = false; 
                    }
                    else {
                        continue;
                ...
                float playerMovementEffect = 0.01f;  
            ...
            if (isEnemy[i]) {
                ...
                velocity[i] = toCenter * speed[i]; // Set velocity towards center 
                ...
                for (int j = 2; j < list_size; j++) {
                    if (isProjectile[j] && isBullet[j]) {
                        ...
                        if (distanceFromEnemies < radius[j] + radius[i]) {
                            ...
                            score ++;
							printf("Score: %d\n", score);
							speed[j] = 0.5 + rand01() * 0.5;  // Random speed between 0.5 and 1.0
    ...
    void shootSphere(vec3 direction, float speed) { 
    ...
    int findOrbitingSphere() const {
        for (int i = 0; i < list_size; i++) {
            if (!isProjectile[i] && center[i].y() > 0) { 
                return i;
            }
        }
        return -1;  // Return -1 if no sphere is found
    }
