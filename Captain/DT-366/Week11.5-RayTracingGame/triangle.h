...
class triangles {
public:
    vec3 p1[200], p2[200], p3[200]; // Actual positions
    vec3 original_p1[200], original_p2[200], original_p3[200]; // Original positions relative to the center
    vec3 albedo[200];
    float fuzz[200];
    float ref_idx[200];
    int mat_type[200];
    float deltaTime[200]; // Time between frames
    int list_size;
    int ramiel_mat_idx = 2;
    vec3 ramiel_albedo = vec3(0.0, 0.0, 0.8);
    vec3 initial_center = vec3(0.0, 0.0, 0.0); // Center of circular motion
    vec3 ramiel_center; // Center of Ramiel
    float radius = 800.0; // Radius from the center
    ...
    triangles() {
        ...
        float plane_size = 10000.0; // Define the size of the plane
        ...
        albedo[idx] = vec3(1.0, 1.0, 1.0); // Grey color for the plane
        fuzz[idx] = 0;
        mat_type[idx] = 8; // Assuming material type 1 for simple diffuse
        idx++;
        ...
        for (int i = 0; i < 40; i++) {
            ...
            float distance = distrRadius(eng); // Distance now ranges from 800 to 1000
            ...
            vec3 baseCorner3 = pillarBaseCenter + vec3(0, 0, baseHalfSize * 1.732); // Equilateral triangle
        ...
        float s = 250.0;  // Side length of the square base
        float half_s = s / 2;
    ...
    // Triangle 1 - First side
        p1[idx] = vec3(half_s, 0, -half_s);
        p2[idx] = vec3(half_s, 0, half_s); // Order reversed
        p3[idx] = apexDown;
        albedo[idx] = ramiel_albedo;
        fuzz[idx] = 0.0;
        mat_type[idx] = ramiel_mat_idx;
    ...
    void update_position(float deltaTime) {
        angle += 0.1 * deltaTime; // Adjust rotation speed here
        ...
        // Assuming indices 0 and 1 are your first two triangles:
